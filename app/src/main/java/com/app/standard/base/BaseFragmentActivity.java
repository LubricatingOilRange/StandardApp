...
public abstract class BaseFragmentActivity extends BaseActivity implements FragmentActivityImpl {
    private boolean isSkipBackStack = true;//当前是否执行跳远 返回
    ...
    public void replaceFragment(@NonNull String replaceFragTag, boolean isAddBackStack, boolean isShowAnim, boolean isBackLeft) {
        ...
        if (isShowAnim) {
            if (isBackLeft) {
                //如果往左进栈 --- 左进右出
                transaction.setCustomAnimations(getInLeftAnim(), getOutRightAnim(), getInRightAnim(), getOutLeftAnim());
            } else {
                //如果往右进栈 --- 右进左出
                transaction.setCustomAnimations(getInRightAnim(), getOutLeftAnim(), getInLeftAnim(), getOutRightAnim());
            }
        }
    ...
    public void addFragment(@NonNull String curFragTag, @NonNull String nextFragTag, boolean isShowAnim, boolean isShowLeft) {
        ...
        if (isShowAnim) {
            if (isShowLeft) {
                //如果往左切换 --- 左进右出
                transaction.setCustomAnimations(getInLeftAnim(), getOutRightAnim(), 0, 0);
            } else {
                //如果往右切换 --- 右进左出
                transaction.setCustomAnimations(getInRightAnim(), getOutLeftAnim(), 0, 0);
            }
        }
